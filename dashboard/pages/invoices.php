            ...
            <form method="GET" class="flex flex-wrap gap-4 items-end">
                ...
                <div>
                    ...
                    <select id="status" name="status" class="border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="sent" <?= $status_filter === 'sent' ? 'selected' : '' ?>>Envoyée</option>
                        <option value="paid" <?= $status_filter === 'paid' ? 'selected' : '' ?>>Payée</option>
                        <option value="overdue" <?= $status_filter === 'overdue' ? 'selected' : '' ?>>En retard</option>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <?php if (empty($invoices)): ?>
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 mb-4">Créez votre première facture pour commencer.</p>
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div>
                                            ...
                                            <div class="text-sm text-gray-500"><?= date('d M Y', strtotime($invoice['created_at'])) ?></div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <?php if ($invoice['discount_amount'] > 0): ?>
                                            <div class="text-sm text-gray-500">Remise : <?= number_format($invoice['discount_amount'], 2) ?> DH</div>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                            ...
                                            switch ($invoice['status']) {
                                                ...
                                                case 'sent': echo 'Envoyée'; break;
                                                case 'paid': echo 'Payée'; break;
                                                case 'overdue': echo 'En retard'; break;
                                                case 'cancelled': echo 'Annulée'; break;
                                                default: echo ucfirst($invoice['status']);
                                            }
                                            ?>
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-3">
                                            <button class="view-invoice-btn p-1 hover:bg-blue-50 rounded-full" data-id="<?= $invoice['id'] ?>" title="Voir la facture">
                                            ...
                                            <button class="edit-invoice-btn p-1 hover:bg-emerald-50 rounded-full" data-id="<?= $invoice['id'] ?>" title="Modifier la facture">
                                            ...
                                            <button class="download-invoice-btn p-1 hover:bg-violet-50 rounded-full" data-id="<?= $invoice['id'] ?>" title="Télécharger la facture">
                                            ...
                                            <?php if ($invoice['status'] !== 'paid'): ?>
                                                <button class="mark-paid-btn p-1 hover:bg-teal-50 rounded-full" data-id="<?= $invoice['id'] ?>" title="Marquer comme payée">
                                            ...
                                            <?php endif; ?>
                                            <button class="send-invoice-btn p-1 hover:bg-sky-50 rounded-full" data-id="<?= $invoice['id'] ?>" title="Envoyer la facture">
                                            ...
                                            <button class="delete-invoice-btn p-1 hover:bg-red-50 rounded-full" data-id="<?= $invoice['id'] ?>" title="Supprimer la facture">
