...
function handlePost($pdo) {
    ...
    try {
        $required_fields = ['patient_id', 'service_id', 'dentist_id', 'appointment_date', 'appointment_time'];
        ...
        // Get service details
        $stmt = $pdo->prepare("
            SELECT duration, price FROM services WHERE id = ? AND clinic_id = ?
        ");
        $stmt->execute([$_POST['service_id'], $clinic_id]);
        $service = $stmt->fetch();
        ...
        ");
        $stmt->execute([$_POST['dentist_id'], $_POST['appointment_date'], $_POST['appointment_time']]);
        $conflict = $stmt->fetch();
        ...
        $stmt = $pdo->prepare("
            INSERT INTO appointments (
                clinic_id, patient_id, service_id, dentist_id, appointment_date, 
...
function handlePut($pdo) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, 'appointment_updated', 'Appointment updated', 
                    CONCAT('Appointment #', ?, ' was updated'), NOW())
        ");
        $stmt->execute([$clinic_id, $input['id']]);
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, 'appointment_cancelled', 'Appointment cancelled', 
                    CONCAT('Appointment #', ?, ' was cancelled'), NOW())
        ");
        $stmt->execute([$clinic_id, $input['id']]);
